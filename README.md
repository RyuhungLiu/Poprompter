...
- **严格同步** — 默认开启，多显示端跟随同一个同步源的滚动位置
- **纯文本遥控** — 独立的 `/k` 路径，AJAX 轮询模式，兼容 Kindle iPhone 4/5 等老旧设备
- **丰富调控** — 滚动速度、画面占比、镜像翻转、颜色反转等
- **屏幕回传** — 控制端实时查看显示端播放进度、当前整屏内容和基于实际滚动的预计剩余时间
- **稿件高亮** — 播放时控制端在稿件区域内高亮显示端当前可见文字，并锁定编辑
- **舞台适配** — 支持参考线位置、文本对齐、到尾自动暂停等提词细节控制
...
快捷键在控制端和纯净遥控器中均可用。滚动速度范围为 1.0 到 5.0，纯净遥控器也提供 0.1 步进的速度加减按钮，便于只拿遥控端时临场微调。
